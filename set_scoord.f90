...
!    input parameters
      ...
      integer, intent( in) :: Vstretching
      real(8), intent( in) :: theta_s    
      real(8), intent( in) :: theta_b    
      real(8), intent( in) :: Tcline     
      real(8), intent( in) :: hmin       
!    output parameters
      real(8), intent(out) :: hc       
      real(8), intent(out) :: sc_w(0:N)       
      real(8), intent(out) :: sc_r(1:N)  
      real(8), intent(out) :: Cs_w(0:N)       
      real(8), intent(out) :: Cs_r(1:N)  
...
!
      ...
      real(8) :: Aweight, Bweight, Cweight, Cbot, Csur, Hscale
      real(8) :: ds, exp_bot, exp_sur, sc_r2, sc_w2
...
!
        IF (theta_s.ne.0.0d0) THEN
          cff1=1.0d0/SINH(theta_s)
        ...
        DO k=1,N
          sc_w(k)=ds*dble(k-N)
          sc_r(k)=ds*(dble(k-N)-0.5d0)
          IF (theta_s.ne.0.0d0) THEN
            Cs_w(k)=(1.0d0-theta_b)*                   &
     &                          cff1*SINH(theta_s*                  &
     &                                    sc_w(k))+         &
     &                          theta_b*                            &
     &                          (cff2*TANH(theta_s*                 &
...
!
        ...
        DO k=N-1,1,-1
          sc_w2=ds*dble(k-N)
          sc_w(k)=sc_w2
          IF (theta_s.gt.0.0d0) THEN
            Csur=(1.0d0-COSH(theta_s*sc_w2))/                       &
     &           (COSH(theta_s)-1.0d0)
            IF (theta_b.gt.0.0d0) THEN
              Cbot=SINH(theta_b*(sc_w2+1.0d0))/                     &
     &             SINH(theta_b)-1.0d0
              Cweight=(sc_w2+1.0d0)**Aweight*                           &
     &                (1.0d0+(Aweight/Bweight)*                        &
     &                        (1.0d0-(sc_w2+1.0d0)**Bweight))
              Cs_w(k)=Cweight*Csur+(1.0d0-Cweight)*Cbot
            ELSE
              Cs_w(k)=Csur
            END IF
          ELSE
...
!
        DO k=1,N
          sc_r2=ds*(dble(k-N)-0.5d0)
          sc_r(k)=sc_r2
          IF (theta_s.gt.0.0d0) THEN
            Csur=(1.0d0-COSH(theta_s*sc_r2))/                       &
     &           (COSH(theta_s)-1.0d0)
            IF (theta_b.gt.0.0d0) THEN
              Cbot=SINH(theta_b*(sc_r2+1.0d0))/                     &
     &             SINH(theta_b)-1.0d0
              Cweight=(sc_r2+1.0d0)**Aweight*                           &
     &                (1.0d0+(Aweight/Bweight)*                        &
     &                        (1.0d0-(sc_r2+1.0d0)**Bweight))
              Cs_r(k)=Cweight*Csur+(1.0d0-Cweight)*Cbot
            ELSE
              Cs_r(k)=Csur
            END IF
          ELSE
...
!
        ...
        DO k=N-1,1,-1
          sc_w2=ds*dble(k-N)
          sc_w(k)=sc_w2
          Cbot= LOG(COSH(Hscale*(sc_w2+1.0d0)**exp_bot))/               &
     &          LOG(COSH(Hscale))-1.0d0
          Csur=-LOG(COSH(Hscale*ABS(sc_w2)**exp_sur))/                   &
     &          LOG(COSH(Hscale))
          Cweight=0.5d0*(1.0d0-TANH(Hscale*(sc_w2+0.5d0)))
          Cs_w(k)=Cweight*Cbot+(1.0d0-Cweight)*Csur
        END DO
        sc_w(0)=-1.0d0
        Cs_w(0)=-1.0d0
!
        DO k=1,N
          sc_r2=ds*(dble(k-N)-0.5d0)
          sc_r(k)=sc_r2
          Cbot= LOG(COSH(Hscale*(sc_r2+1.0d0)**exp_bot))/               &
     &          LOG(COSH(Hscale))-1.0d0
          Csur=-LOG(COSH(Hscale*ABS(sc_r2)**exp_sur))/                   &
     &          LOG(COSH(Hscale))
          Cweight=0.5d0*(1.0d0-TANH(Hscale*(sc_r2+0.5d0)))
          Cs_r(k)=Cweight*Cbot+(1.0d0-Cweight)*Csur
        END DO
!
!-----------------------------------------------------------------------
