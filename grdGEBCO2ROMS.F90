...
!      integer, parameter :: spherical  = 0   
     ...
     ! GEBCO NetCDF bathymetry file     
      character(len=*), parameter :: BATH_FILE =                         &
     &     "D:/Documents/GIS_data/GEBCO_2014_2D/GEBCO_2014_2D.nc"
      character(len=*), parameter :: GRID_FILE  =                        &
     &     "./output/CT_0.08_grd.nc"
     
     ! Set lat, lon at specific station. 
      real(8), parameter :: s_lat = -18.0d0
      real(8), parameter :: e_lat = 30.0d0
      real(8), parameter :: s_lon = 91.0d0
      real(8), parameter :: e_lon = 156.0d0
      
     ! Set ROMS grid resolution
!      real(8), parameter :: RESOLUTION = 24.0d0 ! 1/24 degree resolution
      real(8), parameter :: RESOLUTION = 12.0d0 ! 1/12 degree resolution
      ! Set minimum depth
      real(8), parameter :: hmin = 5.0d0
      
      ! Interporation methode
...
!
      ...
      do i=1, N_xi_rho
        do j=1, N_eta_rho
          pm(i,j) = 1.0d0/distance( lat_rho(i,j)*d_lat,          &
...
!---- Read GEBCO grid netCDF file --------------------------------
      write(*,*) "OPEN: ", BATH_FILE
      
      ! Open NetCDF grid file
      call check( nf90_open(BATH_FILE, nf90_nowrite, ncid) )
      ! Get dimension data
...
!---- Write ROMS grid netCDF file --------------------------------
      ...
      call check( nf90_open(GRID_FILE, NF90_WRITE, ncid) )
