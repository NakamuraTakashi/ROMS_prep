function otps2frc_v5_tpxo9_atlas30(gfile,base_date,pred_date,ofile,model_dir,dname)
...
conList = {'m2','s2','n2','k2','k1','o1','p1','q1','m4','ms4','mn4','2n2'};
num_constituents = length(conList);
...
z_amp= zeros(num_constituents , size(lon,1),size(lon,2));
u_amp= zeros(num_constituents , size(lon,1),size(lon,2));
v_amp= zeros(num_constituents , size(lon,1),size(lon,2));
z_phase= zeros(num_constituents , size(lon,1),size(lon,2));
u_phase= zeros(num_constituents , size(lon,1),size(lon,2));
v_phase= zeros(num_constituents , size(lon,1),size(lon,2));

for j = 1:num_constituents 
    fid = fopen('Model_tmp','w');
    fprintf(fid,'%s\n', [model_dir '/h_' conList{j} '_tpxo9_atlas_30']);
    fprintf(fid,'%s\n', [model_dir '/u_' conList{j} '_tpxo9_atlas_30']);
    fprintf(fid,'%s\n', [model_dir '/grid_tpxo9_atlas_30']);
    fclose(fid);

    [z_amp(j,:,:),z_phase(j,:,:),Depth,con]=tmd_extract_HC('Model_tmp',lat,lon,vars{1},[]);
    [u_amp(j,:,:),u_phase(j,:,:),Depth,con]=tmd_extract_HC('Model_tmp',lat,lon,vars{2},[]);
    [v_amp(j,:,:),v_phase(j,:,:),Depth,con]=tmd_extract_HC('Model_tmp',lat,lon,vars{3},[]);
end
conList
...
%
%
 write_roms_otps_ncfile_v2( Tide, gfile, ofile,dname);
