...
!!!=== ver 2015/12/09   Copyright (c) 2014-2015 Takashi NAKAMURA  =====

    PROGRAM iniHYCOM2ROMS
      use netcdf
      use mod_calendar
...
! -------------------------------------------------------------------------
      ...
      integer, parameter :: itime = 15       ! Frame number of OCEAN_FILE
      
      integer, parameter :: mode = 1           ! mode=1, linear intrtporation
      
     ! NetCDF file     
      character(len=*), parameter :: GRID_FILE = "D:/ROMS/Yaeyama/Data/Yaeyama1_grd_v8.nc"
      character(len=*), parameter :: OCEAN_FILE  = &
     &     "D:/ROMS/Yaeyama/Data/HYCOM_extracted_data/Yaeyama1_HYCOM_extracted_1304.nc"

      character(len=*), parameter :: INI_FILE  = "D:/ROMS/Yaeyama/Data/Yaeyama1_ini_Nz30_HYCOM_130415.nc"
...
! -------------------------------------------------------------------------
      ...
      real(8) :: bry_time(1)            ! Ocean time
      real(8), allocatable :: zeta_west (:,:)   ! free-surface (meter)
      real(8), allocatable :: zeta_east (:,:)   ! free-surface (meter)
      real(8), allocatable :: zeta_south(:,:)   ! free-surface (meter)
      real(8), allocatable :: zeta_north(:,:)   ! free-surface (meter)
      real(8), allocatable :: ubar_west (:,:)   ! vertically integrated u-momentum component (meter second-1)
      real(8), allocatable :: ubar_east (:,:)   ! vertically integrated u-momentum component (meter second-1)
      real(8), allocatable :: ubar_south(:,:)   ! vertically integrated u-momentum component (meter second-1)
      real(8), allocatable :: ubar_north(:,:)   ! vertically integrated u-momentum component (meter second-1)
      real(8), allocatable :: vbar_west (:,:)   ! vertically integrated v-momentum component (milibar=hPa)
      real(8), allocatable :: vbar_east (:,:)   ! vertically integrated v-momentum component (milibar=hPa)
      real(8), allocatable :: vbar_south(:,:)   ! vertically integrated v-momentum component (milibar=hPa)
      real(8), allocatable :: vbar_north(:,:)   ! vertically integrated v-momentum component (milibar=hPa)
      real(8), allocatable :: u_west (:,:,:)    ! u-momentum component (meter second-1)
      real(8), allocatable :: u_east (:,:,:)    ! u-momentum component (meter second-1)
      real(8), allocatable :: u_south(:,:,:)    ! u-momentum component (meter second-1)
      real(8), allocatable :: u_north(:,:,:)    ! u-momentum component (meter second-1)
      real(8), allocatable :: v_west (:,:,:)    ! v-momentum component (meter second-1)
      real(8), allocatable :: v_east (:,:,:)    ! v-momentum component (meter second-1)
      real(8), allocatable :: v_south(:,:,:)    ! v-momentum component (meter second-1)
      real(8), allocatable :: v_north(:,:,:)    ! v-momentum component (meter second-1)
      real(8), allocatable :: temp_west (:,:,:) ! potential temperature (Celsius)
      real(8), allocatable :: temp_east (:,:,:) ! potential temperature (Celsius)
      real(8), allocatable :: temp_south(:,:,:) ! potential temperature (Celsius)
      real(8), allocatable :: temp_north(:,:,:) ! potential temperature (Celsius)
      real(8), allocatable :: salt_west (:,:,:) ! salinity (psu)    
      real(8), allocatable :: salt_east (:,:,:) ! salinity (psu)    
      real(8), allocatable :: salt_south(:,:,:) ! salinity (psu)    
      real(8), allocatable :: salt_north(:,:,:) ! salinity (psu)    
...
! --------------------------------------------------------
      ...
      do i=1,N_xi_u
        do j=1,N_eta_u
          do k=1,N_s_rho
            ubar(i,j)= ubar(i,j)+u(i,j,k)*abs(Cs_w(k)-Cs_w(k+1))
          enddo
        enddo
      enddo
      vbar(:,:)=0.0d0
      do i=1,N_xi_v
        do j=1,N_eta_v
          do k=1,N_s_rho
            vbar(i,j)= vbar(i,j)+v(i,j,k)*abs(Cs_w(k)-Cs_w(k+1))
          enddo
        enddo
      enddo
