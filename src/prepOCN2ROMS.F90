...
! ---------------------------------------------------------------------
  ...
  integer :: SNWE(4)
  character(256) :: HIS_prefix   ! used in HIS output
  character(256) :: INI_prefix   ! used in INI output
  integer :: INIyear, INImonth, INIday, INIhour  ! initial time for INI output (&ini)
#if defined NAOTIDE || defined NAOTIDEJ
  ! ----- NAOTIDE tide configuration (ported from iniOCN2ROMS) -----
  real(8), allocatable :: zeta_tide(:,:)   ! free-surface by tide (meter)
  integer, parameter :: itmode = 2  ! 1=geocentric, 2=pure ocean, 3=radial loading tide
  integer, parameter :: lpmode = 1  ! 1=long-period ocean tide map, 2=equilibrium tide
...
# endif
  ...
  character(256), parameter :: MRICOM_VAR_NAME(5) = (/ &
     "zos   " &  !  1
    ,"uo    " &  !  2 
    ,"vo    " &  !  3
    ,"thetao" &  !  4 
...
# endif
#endif
#if defined MRICOM_MODEL
  namelist/mricom/MRICOM_data_dir
#endif
  ...
  namelist/his/HIS_prefix
  namelist/ini/INI_prefix, INIyear, INImonth, INIday, INIhour
...
#if defined INI_MODE
! Initial-condition time comes from the &ini block (INIyear/INImonth/INIday/INIhour),
! overriding &sdate/&edate. A one-day window is used and Nt is clamped to 1 later, so
! the single initial step is the first donor time at/after the requested date.
  Syear = INIyear ; Smonth = INImonth ; Sday = INIday
  call jd(INIyear, INImonth, INIday, jdate_Start)
  call cdate(jdate_Start + 1, Eyear, Emonth, Eday)
#endif
...
! ---------------- bry_time check ----------------------------------
  ...
  do
    ...
    if(iyear==Eyear .and. imonth==Emonth .and. iday==Eday) then
      ...
      if( Nt <= 0 ) then
        write(*,*) "*** ERROR: No JCOPE-T data file found for the requested period."
...
!    write(*,*) 'FILE ', iNC, trim( NC(iNC)%MRICOM_FILE(1) ) !!!!!!!!!!!!!!!!
  ...
  write(*,*) "Total time step: ", Nt
  if( Nt <= 0 ) then
    write(*,*) "*** ERROR: No MOVE-JPN data file found for the requested period."
    write(*,*) "***   Check the date (&ini for INI_MODE, or &sdate/&edate) and"
...
!-Write ROMS boundary conditions netCDF file --------------------------------
  ...
  ! ===== setup phase: target bounds + (A) array allocation per active boundary =====
  do ibry=1,4
  ...
  ! ===== time loop (outer); boundaries (inner) write into one OUT_FILE =====
  do itime=1,Nt
    iNCm = iNC
    iNC  = iNCt(itime)
  ...
  !---- Load HYCOM netCDF grid cooredinate --------------------------------
    ...
    do ibry=1,4
      ...
      ! ----- restore this boundary's target bounds + alias (A) pointers -----
...
# endif
  ...
  write(*,*) '***************** boundary extraction complete! *****************'
