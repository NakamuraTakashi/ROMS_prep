...
! -------------------------------------------------------------------------
      ...
      character(10) :: BRY_suffix   = "_200001.nc"
...
!          input parameters
     ...
     &      )
      ...
      call check( nf90_open(trim( BRY_FILE ), NF90_WRITE, ncid) )
...
! --- Linear interporation --------------------------------
        ...
        if (mode==1) then
          ...
          write(*,*) 'Linear Interporation: zeta_west'
          call LinearInterpolation2D_grid2(Im, Jm, lon, lat               &
     &                    , surf_el(:,:,1), -5.0d0, 5.0d0                 &
     &                    , 1, N_eta_rho                                  &
     &                    , lon_rho(1,:), lat_rho(1,:)                    &
     &                    , zeta(1,:) )
          write(*,*) 'Linear Interporation: zeta_east'
          call LinearInterpolation2D_grid2(Im, Jm, lon, lat               &
     &                    , surf_el(:,:,1), -5.0d0, 5.0d0                 &
     &                    , 1, N_eta_rho                                  &
     &                    , lon_rho(N_xi_rho,:), lat_rho(N_xi_rho,:)      &
     &                    , zeta(N_xi_rho,:) )
          write(*,*) 'Linear Interporation: zeta_south'
          call LinearInterpolation2D_grid2(Im, Jm, lon, lat               &
     &                    , surf_el(:,:,1), -5.0d0, 5.0d0                 &
     &                    , N_xi_rho, 1                                   &
     &                    , lon_rho(:,1), lat_rho(:,1)                    &
     &                    , zeta(:,1) )
          write(*,*) 'Linear Interporation: zeta_north'
          call LinearInterpolation2D_grid2(Im, Jm, lon, lat               &
     &                    , surf_el(:,:,1), -5.0d0, 5.0d0                 &
     &                    , N_xi_rho, 1                                   &
     &                    , lon_rho(:,N_eta_rho), lat_rho(:,N_eta_rho)    &
     &                    , zeta(:,N_eta_rho) )
     

          write(*,*) 'Linear Interporation: temp_west'
     ...
     &                    , z_r(1,:,:)                                    &
     &                    , temp(1,:,:) )
          write(*,*) 'Linear Interporation: temp_east'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_temp(:,:,:,1),  0.0d0, 40.0d0           &
     &                    , 1, N_eta_rho, N_s_rho                         &
     &                    , lon_rho(N_xi_rho,:), lat_rho(N_xi_rho,:)      &
     &                    , z_r(N_xi_rho,:,:)                             &
     &                    , temp(N_xi_rho,:,:) )
          write(*,*) 'Linear Interporation: temp_south'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_temp(:,:,:,1),  0.0d0, 40.0d0           &
     &                    , N_xi_rho, 1, N_s_rho                          &
     &                    , lon_rho(:,1), lat_rho(:,1)                    &
     &                    , z_r(:,1,:)                                    &
     &                    , temp(:,1,:) )
          write(*,*) 'Linear Interporation: temp_north'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_temp(:,:,:,1),  0.0d0, 40.0d0           &
     &                    , N_xi_rho, 1, N_s_rho                          &
     &                    , lon_rho(:,N_eta_rho), lat_rho(:,N_eta_rho)    &
     ...
     &                    , temp(:,N_eta_rho,:) )
          ...
          write(*,*) 'Linear Interporation: salt_west'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     ...
     &                    , z_r(1,:,:)                                    &
     &                    , salt(1,:,:) )
          write(*,*) 'Linear Interporation: salt_east'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , salinity(:,:,:,1),  20.0d0, 36.0d0            &
     &                    , 1, N_eta_rho, N_s_rho                         &
     &                    , lon_rho(N_xi_rho,:), lat_rho(N_xi_rho,:)      &
     &                    , z_r(N_xi_rho,:,:)                             &
     &                    , salt(N_xi_rho,:,:) )
          write(*,*) 'Linear Interporation: salt_south'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , salinity(:,:,:,1),  20.0d0, 36.0d0            &
     &                    , N_xi_rho, 1, N_s_rho                          &
     &                    , lon_rho(:,1), lat_rho(:,1)                    &
     &                    , z_r(:,1,:)                                    &
     &                    , salt(:,1,:) )
          write(*,*) 'Linear Interporation: salt_north'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , salinity(:,:,:,1),  20.0d0, 36.0d0            &
     &                    , N_xi_rho, 1, N_s_rho                          &
     &                    , lon_rho(:,N_eta_rho), lat_rho(:,N_eta_rho)    &
     ...
     &                    , salt(:,N_eta_rho,:) )
          ...
          write(*,*) 'Linear Interporation: u_west'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     ...
     &                    , z_r(1,:,:)                                    &
     &                    , vll_u(1,:,:) )
          write(*,*) 'Linear Interporation: u_east'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , 1, N_eta_u, N_s_rho                           &
     &                    , lon_u(N_xi_u,:), lat_u(N_xi_u,:)              &
     &                    , z_r(N_xi_u,:,:)                               &
     &                    , ull_u(N_xi_u,:,:) )
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     ...
     &                    , z_r(N_xi_u,:,:)                               &
     &                    , vll_u(N_xi_u,:,:) )
          write(*,*) 'Linear Interporation: u_south'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , N_xi_u, 1, N_s_rho                            &
     &                    , lon_u(:,1), lat_rho(:,1)                      &
     ...
     &                    , z_r(:,1,:)                                    &
     &                    , vll_u(:,1,:) )
          write(*,*) 'Linear Interporation: u_north'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , N_xi_u, 1, N_s_rho                            &
     &                    , lon_u(:,N_eta_u), lat_rho(:,N_eta_u)          &
     &                    , z_r(:,N_eta_u,:)                              &
     &                    , ull_u(:,N_eta_u,:) )
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     ...
     &                    , vll_u(:,N_eta_u,:) )
          ...
          write(*,*) 'Linear Interporation: v_west'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , 1, N_eta_v, N_s_rho                           &
     &                    , lon_v(1,:), lat_v(1,:)                        &
     ...
     &                    , z_r(1,:,:)                                    &
     &                    , ull_v(1,:,:) )
          write(*,*) 'Linear Interporation: v_east'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , 1, N_eta_v, N_s_rho                           &
     &                    , lon_v(N_xi_v,:), lat_u(N_xi_v,:)              &
     &                    , z_r(N_xi_v,:,:)                               &
     &                    , vll_v(N_xi_v,:,:) )
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , 1, N_eta_v, N_s_rho                           &
     &                    , lon_v(N_xi_v,:), lat_v(N_xi_v,:)              &
     &                    , z_r(N_xi_v,:,:)                               &
     &                    , ull_v(N_xi_v,:,:) )
          write(*,*) 'Linear Interporation: v_south'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , N_xi_v, 1, N_s_rho                            &
     &                    , lon_v(:,1), lat_rho(:,1)                      &
     ...
     &                    , z_r(:,1,:)                                    &
     &                    , ull_v(:,1,:) )
          write(*,*) 'Linear Interporation: v_north'
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,1),  -5.0d0, 5.0d0              &
     &                    , N_xi_v, 1, N_s_rho                            &
     &                    , lon_v(:,N_eta_v), lat_rho(:,N_eta_v)          &
     &                    , z_r(:,N_eta_v,:)                              &
     &                    , vll_v(:,N_eta_v,:) )
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,1),  -5.0d0, 5.0d0              &
