...
! -------------------------------------------------------------------------
      ...
      real(8), parameter :: DT   = 30.0d0   ! Time interval (min)
     ...
     &     "D:/ROMS/Yaeyama/Data/Yaeyama1_bry_Nz30_HYCOMpNAO_1304.nc"
      character(len=*), parameter :: INI_FILE  =                        &
     &     "D:/ROMS/Yaeyama/Data/Yaeyama1_ini_Nz30_HYCOMpNAO_1309.nc"
...
!      ! Allocate variable
      ...
      allocate(lon(Im))
      allocate(depth(Nz))
      allocate(surf_el(Im,Jm,2))
      allocate(water_temp(Im,Jm,Nz,2))
      allocate(salinity(Im,Jm,Nz,2))
      allocate(water_u(Im,Jm,Nz,2))
      allocate(water_v(Im,Jm,Nz,2))
...
! --------------------------------------------------------
        ...
        ocean_time(2) = time_all(itime+1)*24.0d0*60.0d0*60.0d0  !! day -> second
...
! --- Linear interporation --------------------------------
     ...
     &                    , ull_v(:,:,:) )
          ...
          call LinearInterpolation2D_grid2(Im, Jm, lon, lat               &
     &                    , surf_el(:,:,2), -5.0d0, 5.0d0                 &
     &                    , N_xi_rho, N_eta_rho, lon_rho, lat_rho, zeta(:,:,2) )
        
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_temp(:,:,:,2),  0.0d0, 40.0d0           &
     &                    , N_xi_rho, N_eta_rho, N_s_rho                  &
     &                    , lon_rho, lat_rho, z_r                         &
     &                    , temp(:,:,:,2) )
     
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , salinity(:,:,:,2),  20.0d0, 36.0d0            &
     &                    , N_xi_rho, N_eta_rho, N_s_rho                  &
     &                    , lon_rho, lat_rho, z_r                         &
     &                    , salt(:,:,:,2) )

          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_u(:,:,:,2),  -5.0d0, 5.0d0              &
     &                    , N_xi_u, N_eta_u, N_s_rho                      &
     &                    , lon_u, lat_u, z_r_u                           &
     &                    , ull_u(:,:,:) )
          call LinearInterpolation3D_grid3(Im, Jm, Nz, lon, lat, depth    &
     &                    , water_v(:,:,:,2),  -5.0d0, 5.0d0              &
     &                    , N_xi_u, N_eta_u, N_s_rho                      &
     &                    , lon_u, lat_u, z_r_u                           &
     &                    , vll_u(:,:,:) )
          do i=1,N_xi_u
            do j=1,N_eta_u
              do k=1,N_s_rho
                u(i,j,k,2) = ull_u(i,j,k)*cosAu(i,j)                      &
                            +vll_u(i,j,k)*sinAu(i,j)
              enddo
            enddo
          enddo
     ...
     &                    , ull_v(:,:,:) )
          do i=1,N_xi_v
            do j=1,N_eta_v
              do k=1,N_s_rho
                v(i,j,k,2) =-ull_v(i,j,k)*sinAv(i,j)                      &
                            +vll_v(i,j,k)*cosAv(i,j)
              enddo
            enddo
          enddo
         ...
         do i=1,N_xi_u
           do j=1,N_eta_u
             do k=1,N_s_rho
               ...
               temp(i,j,k,1) = potemp
               CALL POTMP(press,temp(i,j,k,2),salt(i,j,k,2),ref_press,potemp)
               temp(i,j,k,2) = potemp
             enddo
           enddo
         enddo
...
! --------------------------------------------------------
        ...
        idt=0
        do
          ocean_time(3) = ocean_time(1)+DT*60.0d0*idt
          if(ocean_time(3)>=ocean_time(2)) exit
          write(*,*) 'time (days) = ', ocean_time(3)/86400.0d0
          
          wf=(ocean_time(3)-ocean_time(1))/(ocean_time(2)-ocean_time(1))
          
          zeta(:,:,3)=(1.0d0-wf)*zeta(:,:,1)+wf*zeta(:,:,2)
          temp(:,:,:,3)=(1.0d0-wf)*temp(:,:,:,1)+wf*temp(:,:,:,2)
          salt(:,:,:,3)=(1.0d0-wf)*salt(:,:,:,1)+wf*salt(:,:,:,2)
          u(:,:,:,3)=(1.0d0-wf)*u(:,:,:,1)+wf*u(:,:,:,2)
          v(:,:,:,3)=(1.0d0-wf)*v(:,:,:,1)+wf*v(:,:,:,2)
          
          ubar(:,:,3)=0.0d0
          do i=1,N_xi_u
            do j=1,N_eta_u
              do k=1,N_s_rho
                ubar(i,j,3)= ubar(i,j,3)+u(i,j,k,3)*abs(Cs_w(k-1)-Cs_w(k))
              enddo
            enddo
          enddo
          vbar(:,:,3)=0.0d0
          do i=1,N_xi_v
            do j=1,N_eta_v
              do k=1,N_s_rho
                vbar(i,j,3)= vbar(i,j,3)+v(i,j,k,3)*abs(Cs_w(k-1)-Cs_w(k))
              enddo
            enddo
          enddo
...
!---- NAOTIDE calculation --------------------------------
          ...
          jtime = dble(Smjd) + ocean_time(3)/86400.0d0
!          write(*,*) jtime, Smjd
          do i=1,N_xi_rho
            do j=1,N_eta_rho
...
#ifdef RAMP_TIDES
      ramp=TANH((ocean_time(3)/86400.0d0)/1.0d0)
#else
      ramp=1.0d0
#endif          
          zeta(:,:,3)=zeta(:,:,3)+ramp*zeta_tide(:,:)
          
!          zeta(:,:,3)=zeta(:,:,3)*mask_rho(:,:)
